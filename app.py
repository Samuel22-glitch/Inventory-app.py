...
def get_drinks():
    ...
    """
    db = get_db()
    cursor = db.execute('SELECT * FROM drinks')
...
def get_drink(drink_id):
    """
    Retrieves a single drink by its ID.
    """
    db = get_db()
...
def create_drink():
    ...
    Expects JSON data with 'name', 'description', and 'price'.
    """
    data = request.get_json()
    if not data or not all(k in data for k in ('name', 'price', 'batch_no', 'expiry_date', 'quantity', 'drink_subtype')):
    ...
    try:
        ...
        db.commit()
        # Return the newly created drink with its ID
        return jsonify({'id': cursor.lastrowid, 'name': name, 'description': description, 'price': price, 'batch_no': batch_no, 'expiry_date': expiry_date, 'quantity': quantity, 'drink_subtype': drink_subtype}), 201
...
def update_drink(drink_id):
    """
    Updates an existing drink by its ID.
    Expects JSON data with 'name', 'description', and 'price'.
    """
    data = request.get_json()
    if not data:
    ...
    try:
        db.execute('UPDATE drinks SET name = ?, description = ?, price = ? WHERE id = ?',
                   (name, description, price, drink_id))
        db.commit()
        return jsonify({'message': 'Drink updated successfully', 'id': drink_id, 'name': name, 'description': description, 'price': price})
    except sqlite3.Error as e:
        return jsonify({'message': f'Database error: {e}'}), 500
...
if __name__ == '__main__':
    ...
    # debug=True allows forzz automatic reloading on code changes and provides a debugger
    app.run(debug=True)
